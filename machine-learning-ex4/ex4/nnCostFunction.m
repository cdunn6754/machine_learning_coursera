...
for i = 1:m;
  ...
  % figure out the right y
  y_current = zeros(num_labels, 1);
  y_current(y(i)) = 1;

  % cost
  J = J + (-y_current' * log(a3) - (1-y_current)' * log(1-a3));


  % Back Prop
  delta_3 = a3 - y_current;
...
Theta1_grad = (1/m)*big_delta_1 + (lambda/m)*Theta1_reg;
Theta2_grad = (1/m)*big_delta_2 + (lambda/m)*Theta2_reg;
